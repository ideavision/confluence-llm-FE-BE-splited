...
class ConfluenceConnector(LoadConnector, PollConnector):
    ...
    def load_credentials(self, credentials: dict[str, Any]) -> dict[str, Any] | None:
        ...
        self.token = credentials.get("confluence_access_token")

        # Construct the appropriate headers for Bearer token authentication
        headers = {"Authorization": f"Bearer {self.token}"}

        if self.is_cloud:
            # For cloud, use the token-based authentication
            self.confluence_client = Confluence(
                url=self.wiki_base, token=self.token, cloud=True
            )
        else:
            # For self-hosted, use the token-based authentication
            self.confluence_client = Confluence(url=self.wiki_base, token=self.token)
        return None
        ...
        for page in batch:
            ...
            if time_filter is None or time_filter(last_modified):
                ...
                if self.labels_to_include:
                    ...
                    label_intersection = self.labels_to_include.intersection(
                        page_labels
                    )
